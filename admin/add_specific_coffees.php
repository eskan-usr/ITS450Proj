...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	...
	if (isset($_POST['category']) && filter_var($_POST['category'], FILTER_VALIDATE_INT, array('min_range' => 1))) {
		...
		echo '<p class="error">Select a category.</p>';
	}
...
<form action="add_specific_coffees.php" method="post" accept-charset="utf-8">
	...
	<fieldset><legend>Fill out the form to add cameras to the site.</legend>
		
		<div class="field"><label for="category"><strong>Camera Options</strong></label><br />
...
// Need the available grind options:
		$grinds = '<option value="cheap">cheap</option><option value="mid-range">mid-range</option><option value="expensive">expensive</option>';
		
		
		

		// Need the caffeinated/decaffeinated options:
		$caf_decaf = '<option value="Camera 1">Camera 1</option><option value="Camera 2">Camera 2</option><option value="Camera 3">Camera 3</option>';
